"""Merge GSMLS .xls broker exports into the canonical 19-col schema, per city.

GSMLS exports (data/<city>/*.xls) are listing dumps: 212 columns, one row per
listing, many non-sales (withdrawn/expired/active). This:
  ...
  - maps to the same 19 columns the notebooks use.

Writes data/<city>/merged.csv. Used by GSMLS-sourced cities (Millburn, Chatham
Twp). Run: python3 src/merge_gsmls.py <city>
"""
...
def load_closed_sales(city: City) -> pd.DataFrame:
    src_dir = city.data.parent
    ...
    print(f"[{city.name}] {len(files)} files, {len(raw):,} rows")
    ...
    closed = closed.sort_values("_mod").drop_duplicates("MlsNum", keep="last")
    print(f"after de-dup to latest per MlsNum: {len(closed):,}")
    return closed


def to_schema(d: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Property Address"] = addr.where(unit.isna(), addr + " #" + unit.fillna(""))
...
if __name__ == "__main__":
    ...
    df = to_schema(load_closed_sales(city))
    df.to_csv(city.data, index=False)
    print(f"wrote {city.data} — {len(df):,} closed sales, {len(df.columns)} columns")
